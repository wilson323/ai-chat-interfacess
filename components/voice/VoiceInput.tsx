...
export function VoiceInput({
  ...
  const { state, startRecording, stopRecording, reset } =
    useVoiceRecorder(config);
  ...
  const handleVoiceError = useCallback(
    (errorType: string, message: string, details?: any) => {
    ...
    async (audioBlob: Blob): Promise<string | null> => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          const errorMessage =
            errorData?.error ||
            `语音识别服务请求失败: ${response.status} ${response.statusText}`;
      ...
      try {
        ...
        if (error instanceof Error) {
          if (error.name === 'NotAllowedError') {
            handleVoiceError(
              'PERMISSION_DENIED',
              '需要麦克风权限才能使用语音功能，请在浏览器设置中允许访问麦克风'
            );
    ...
    <div className={cn('relative space-y-3', className)}>
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className='text-xs text-gray-500 p-2 bg-gray-100 rounded'>
          录音状态: {state.isRecording ? '录音中' : '未录音'} | 转录状态:{' '}
          {isTranscribing ? '转录中' : '空闲'} | 启用状态:{' '}
          {isEnabled ? '已启用' : '未启用'}
        </div>
      )}
      ...
      {showSuccess && lastTranscript ? (
        <SuccessStatus message='识别成功' duration={state.duration} />
      ) : isTranscribing ? (
...
export function FloatingVoiceInput({
  ...
  const { audioLevel } = useAudioLevel(
    state.isRecording ? ({} as MediaStream) : null
  );
