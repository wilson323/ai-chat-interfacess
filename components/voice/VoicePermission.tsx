  ...
  return (
    ...
    >
      ...
      <AlertDescription className='space-y-3'>
        ...
        {permission.state === 'denied' && <PermissionGuide />}
      </AlertDescription>
    </Alert>
  );
}
...
function UnsupportedBrowser({ className }: { className?: string }) {
  return (
    <Alert
      className={cn('border-red-200 bg-red-50 dark:bg-red-950/20', className)}
    >
...
export function PermissionIndicator({
  ...
  return (
    <div className={cn('flex items-center gap-2', className)}>
      ...
      {showText && (
        <span className={cn('text-sm', indicator.color)}>{indicator.text}</span>
      )}
    </div>
  );
}
 ...
 * 权限设置链接组件
 */
export function PermissionSettingsLink({ className }: { className?: string }) {
