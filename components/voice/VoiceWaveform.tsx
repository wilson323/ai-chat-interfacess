  ...
  const barHeights = useMemo(() => {
    ...
    for (let i = 0; i < bars; i++) {
      ...
      const height = Math.max(
        0.1,
        baseHeight * audioLevel * (0.8 + Math.random() * 0.4)
      );
...
export function DynamicWaveform({
  ...
  switch (variant) {
    case 'circular':
      return (
        <CircularWaveform audioLevel={audioLevel} className={baseClassName} />
      );

    case 'pulse':
      return (
        <PulseWaveform audioLevel={audioLevel} className={baseClassName} />
      );

    case 'linear':
      return waveformData ? (
        <LinearWaveform waveformData={waveformData} className={baseClassName} />
      ) : null;

    case 'bars':
    default:
      return (
        <VoiceWaveform audioLevel={audioLevel} className={baseClassName} />
      );
  }
}
