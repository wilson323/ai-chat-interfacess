  ...
  return (
    <NoSSR>
      <ErrorBoundary fallback={<div className={cn("text-red-500 text-xs", className)}>消息渲染出错</div>}>
        <div
          className={cn(
            "relative group max-w-full w-fit min-w-[60px] px-4 py-3 my-2 rounded-2xl shadow-md border border-zinc-200 dark:border-zinc-700/50 bg-gradient-to-br from-white via-zinc-50 to-zinc-100 dark:from-zinc-900 dark:via-zinc-800 dark:to-zinc-900",
          ...
          <ReactMarkdown
            ...
            components={{
              ...
              img({ node, src, alt, ...props }) {
                ...
                const isLoading = imageLoadingStates[processedSrc]?.isLoading ?? true
                const error = imageLoadingStates[processedSrc]?.error ?? false
