...
export function MarkdownMessage({
  ...
  return (
    <ErrorBoundary
      fallback={
        <div className={cn('text-red-500 text-xs', className)}>
          消息渲染出错
        </div>
      }
    >
      ...
      >
        <ReactMarkdown
          ...
          components={{
            p: ({ children }) => (
              <p
                className='font-sans font-medium text-message-base text-text-base dark:text-text-dark-base prose-invert:text-white mb-0 last:mb-0 tracking-tightest p-0 m-0 leading-relaxed drop-shadow-sm'
                style={{
                  letterSpacing: '0.01em',
                  lineHeight: '1.8',
                  textShadow: '0 1px 8px rgba(60,60,60,0.04)',
                }}
              >
                {children}
              </p>
            ...
            strong: ({ children }) => (
              <strong
                className='font-bold text-pantone369-700 dark:text-pantone369-300 prose-invert:text-white tracking-tightest p-0 m-0'
            ...
            a: ({ children, href }) => (
              <a
                ...
                className='text-blue-600 dark:text-blue-400 hover:underline underline-offset-2 p-0 m-0'
              >
                {children}
              </a>
            ),
            ...
            code: ({ children, className }) => (
              <code
                className={`font-mono text-[13.5px] px-2 py-1 rounded text-code dark:text-code-dark border border-gray-200 dark:border-gray-700 ${className || ''} p-0 m-0`}
              >
                {children}
              </code>
            ),
            pre: ({ children }) => (
              <pre className='rounded-lg px-3 py-2 overflow-x-auto border border-gray-200 dark:border-gray-700 my-0 p-0 m-0'>
                <code className='font-mono text-[13.5px] text-code dark:text-code-dark p-0 m-0'>
                  {children}
                </code>
              </pre>
            ),
            ul: ({ children }) => (
              <ul className='list-disc pl-5 space-y-1 marker:text-text-base dark:marker:text-text-dark-base prose-invert:marker:text-white p-0 m-0'>
                {children}
              </ul>
            ),
            ol: ({ children }) => (
              <ol className='list-decimal pl-5 space-y-1 marker:font-mono marker:text-text-base dark:marker:text-text-dark-base prose-invert:marker:text-white p-0 m-0'>
                {children}
              </ol>
            ),
            li: ({ children }) => (
              <li className='pl-1 p-0 m-0 text-text-base dark:text-text-dark-base prose-invert:text-white'>
                {children}
              </li>
            ),
            h1: ({ children }) => (
              <h1 className='text-xl font-bold mt-6 mb-3 pb-2 border-b border-gray-200 dark:border-gray-700 p-0 m-0'>
                {children}
              </h1>
            ),
            h2: ({ children }) => (
              <h2 className='text-lg font-semibold mt-5 mb-2 p-0 m-0'>
                {children}
              </h2>
            ),
            h3: ({ children }) => (
              <h3 className='text-base font-medium mt-4 mb-1.5 p-0 m-0'>
                {children}
              </h3>
            ),
            blockquote: ({ children }) => (
              <blockquote className='border-l-3 border-gray-300 dark:border-gray-600 pl-3 text-gray-600 dark:text-gray-300 italic my-0 p-0 m-0'>
                {children}
              </blockquote>
            ),
            hr: () => (
            ...
            span: ({ children }) => (
              <span
                className='font-sans text-message-base tracking-tightest p-0 m-0'
                style={{ letterSpacing: '0.01em' }}
              >
                {children}
              </span>
            ...
            table: ({ children }) => (
              <div className='overflow-x-auto my-4'>
                <table className='min-w-full border border-collapse border-gray-200 dark:border-gray-700 rounded-md'>
                  {children}
                </table>
              </div>
            ),
            ...
            th: ({ children }) => (
              <th className='px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider border-r border-gray-200 dark:border-gray-700 last:border-r-0'>
                {children}
              </th>
            ),
            td: ({ children }) => (
              <td className='px-4 py-2 text-sm border-r border-gray-200 dark:border-gray-700 last:border-r-0'>
                {children}
              </td>
            ),
          }}
