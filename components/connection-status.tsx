      ...
      try {
        ...
        if (response.ok) {
          ...
          try {
            ...
            setStatus('error');
            const message =
              data.error?.message || `Connection failed: ${response.status}`;
            setErrorMessage(message);
        ...
        if (shouldRetry) {
          ...
          console.log(
            `Will retry connection in ${backoffTime}ms (attempt ${retryCount + 1}/3)`
          );
          setTimeout(() => {
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            ...
            />
            {!isMobile && (
              <span className='text-[10px] sm:text-xs'>{getStatusText()}</span>
            )}
            {isRetrying ? (
          ...
          <div className='space-y-1'>
            ...
            <div className='pt-1'>
              ...
              >
                {isRetrying ? (
                  <>
                    <RefreshCw className='h-2.5 w-2.5 sm:h-3 sm:w-3 animate-spin mr-1' />{' '}
                    {t('retrying')}
                  </>
                ) : (
                  <>
