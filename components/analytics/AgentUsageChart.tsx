...
export default function AgentUsageChart({
  ...
  }, [chartType, groupBy, startDate, endDate, fetchData]);

  // 获取颜色
  const getColor = (agentType: string) => {
    return (
      agentTypeColors[agentType as keyof typeof agentTypeColors] ||
      agentTypeColors.unknown
    );
  };
  ...
  const CustomTooltip = ({
    ...
    payload?: Array<{
      ...
      value: number;
      color: string;
      payload: AgentUsageData;
    }>;
    label?: string;
  }) => {
    if (active && payload && payload.length) {
      const data = payload[0].payload as AgentUsageData;
      return (
        <div className='bg-white p-3 border border-gray-200 rounded-lg shadow-lg'>
          ...
          <p className='text-sm text-gray-600'>
            ...
            <span className='font-medium'>
              {groupBy === 'usage' ? data.usage.toLocaleString() :
               groupBy === 'duration' ? data.duration.toLocaleString() :
               groupBy === 'responseTime' ? data.responseTime.toLocaleString() :
               data.satisfaction.toLocaleString() || 'N/A'}
            </span>
          </p>
          {data.messageCount && (
            <p className='text-sm text-gray-600'>
              消息数: {data.messageCount.toLocaleString()}
            </p>
          )}
        </div>
      );
  ...
  const RadarTooltip = ({
    ...
    active?: boolean;
    payload?: Array<{ payload: RadarData }>;
  }) => {
    if (active && payload && payload.length) {
      const data = payload[0].payload as RadarData;
      return (
        <div className='bg-white p-3 border border-gray-200 rounded-lg shadow-lg'>
          <p className='font-medium text-gray-900 mb-1'>{data.agent}</p>
          <p className='text-sm text-gray-600'>智能体</p>
    ...
    <Card className={className}>
      <CardHeader>
        ...
        <div className='flex flex-wrap gap-4 pt-4'>
          <div className='flex items-center gap-2'>
            <input
              id='agentUsageStartDate'
              name='agentUsageStartDate'
              type='date'
              value={startDate}
            ...
            <input
              id='agentUsageEndDate'
              name='agentUsageEndDate'
              type='date'
              value={endDate}
          ...
          <Select
            value={chartType}
            onValueChange={value => setChartType(value as 'pie' | 'bar' | 'radar')}
          >
          ...
          {chartType !== 'radar' && (
            <Select
              value={groupBy}
              onValueChange={value => setGroupBy(value as 'usage' | 'duration' | 'responseTime' | 'satisfaction')}
            >
