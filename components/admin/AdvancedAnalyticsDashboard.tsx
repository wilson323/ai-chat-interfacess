...
import {
  UserBehaviorAnalytics as UserBehaviorAnalyticsType,
  AgentPerformanceAnalytics as AgentPerformanceAnalyticsType,
  ConversationAnalytics as ConversationAnalyticsType,
  BusinessValueAnalytics as BusinessValueAnalyticsType,
  PredictionAnalytics as PredictionAnalyticsType,
  AnalyticsSummary
} from '@/types/admin';
...
export function AdvancedAnalyticsDashboard() {
  ...
  const [userBehavior, setUserBehavior] = useState<UserBehaviorAnalyticsType | null>(null);
  const [agentPerformance, setAgentPerformance] = useState<AgentPerformanceAnalyticsType | null>(null);
  const [conversation, setConversation] = useState<ConversationAnalyticsType | null>(null);
  const [businessValue, setBusinessValue] = useState<BusinessValueAnalyticsType | null>(null);
  const [prediction, setPrediction] = useState<PredictionAnalyticsType | null>(null);
  ...
  const fetchAnalyticsData = async () => {
    try {
      ...
      if (!summaryResponse.ok) throw new Error('获取数据失败');
      const summaryData = await summaryResponse.json();
      setSummary(summaryData);

      // Fetch individual analytics data in parallel
      const [userBehaviorRes, agentPerformanceRes, conversationRes, businessValueRes, predictionRes] = await Promise.all([
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          {/* Date range buttons */}
          <Button
            variant={dateRange.startDate.getTime() === new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).getTime() ? 'default' : 'outline'}
            size="sm"
          ...
          <Button
            variant={dateRange.startDate.getTime() === new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).getTime() ? 'default' : 'outline'}
            size="sm"
          ...
          <Button
            variant={dateRange.startDate.getTime() === new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).getTime() ? 'default' : 'outline'}
            size="sm"
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        <TabsList className="grid w-full grid-cols-6">
          ...
          <TabsTrigger value="user-behavior" className="flex items-center space-x-2">
          ...
          <TabsTrigger value="business-value" className="flex items-center space-x-2">
                ...
                {userBehavior ? (
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-lg font-bold">{userBehavior.activeUsers}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-medium">平均会话时长</span>
                      <span className="text-lg font-bold">{Math.round(userBehavior.avgSessionDuration / 1000)}秒</span>
                    </div>
                    <div className="flex justify-between items-center">
                ...
                {agentPerformance ? (
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-lg font-bold">{agentPerformance.avgResponseTime}ms</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-medium">平均错误率</span>
                      <Badge variant={agentPerformance.avgErrorRate < 0.05 ? 'default' : 'destructive'}>
                ...
                {businessValue ? (
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-lg font-bold">{businessValue.costAnalysis.totalTokens?.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-medium">总成本</span>
                      <span className="text-lg font-bold">${businessValue.costAnalysis.estimatedCost?.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-medium">投资回报率</span>
                      <Badge variant={businessValue.costAnalysis.roi > 0 ? 'default' : 'destructive'}>
                        {businessValue.costAnalysis.roi.toFixed(1)}%
                      </Badge>
                    </div>
                  </div>
                ) : (
                ...
                {prediction ? (
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-lg font-bold">{prediction.usageTrend.growthRate.toFixed(1)}%</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-medium">预测准确度</span>
                      <Badge variant={prediction.usageTrend.accuracy > 0.8 ? 'default' : 'secondary'}>
                        {(prediction.usageTrend.accuracy * 100).toFixed(0)}%
                      </Badge>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-medium">异常事件</span>
                      <Badge variant={prediction.anomalyDetection.anomalies.length > 0 ? 'destructive' : 'default'}>
                        {prediction.anomalyDetection.anomalies.length} 个
                      </Badge>
                    </div>
                  </div>
                ) : (
        ...
        <TabsContent value="user-behavior">
          {userBehavior ? (
            <UserBehaviorAnalytics data={userBehavior} dateRange={dateRange} />
          ) : (
        ...
        <TabsContent value="agent-performance">
          {agentPerformance ? (
            <AgentPerformanceAnalytics data={agentPerformance} dateRange={dateRange} />
          ) : (
            <Skeleton className="h-96" />
          )}
        </TabsContent>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">总Token使用量: {businessValue.costAnalysis.totalTokens?.toLocaleString()}</p>
                      <p className="text-sm text-gray-600">估计成本: ${businessValue.costAnalysis.estimatedCost?.toFixed(2)}</p>
                    </div>
                    <div>
                      <h4 className="font-semibold mb-2">价值指标</h4>
                      <p className="text-sm text-gray-600">投资回报率: {businessValue.costAnalysis.roi.toFixed(1)}%</p>
                      <p className="text-sm text-gray-600">效率提升: {businessValue.efficiencyMetrics.timeSaved.toFixed(1)}%</p>
                    </div>
                  </div>
                </div>
              </CardContent>
        ...
        <TabsContent value="prediction">
          {prediction ? (
            <PredictionDashboard data={prediction} dateRange={dateRange} />
          ) : (
            <Skeleton className="h-96" />
          )}
        </TabsContent>
