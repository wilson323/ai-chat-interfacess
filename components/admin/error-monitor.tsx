    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {(stats.byType.NETWORK_ERROR || 0) + (stats.byType.EXTERNAL_API_ERROR || 0)}
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center space-x-2">
          <Button
            ...
            size="sm"
          >
            {showStack ? <EyeOff className="h-4 w-4 mr-2" /> : <Eye className="h-4 w-4 mr-2" />}
            {showStack ? '隐藏堆栈' : '显示堆栈'}
          </Button>
        </div>
      </div>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-start space-x-3">
                        <div className="flex-shrink-0">
                          {typeIcons[error.type as keyof typeof typeIcons] || <Bug className="h-4 w-4" />}
                        </div>
              ...
              <Card key={type}>
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      {typeIcons[type as keyof typeof typeIcons] || <Bug className="h-4 w-4" />}
          ...
          <Card className="w-full max-w-2xl max-h-[80vh] overflow-y-auto">
            <CardHeader>
              <CardTitle className="flex items-center space-x-2">
                {typeIcons[selectedError.type as keyof typeof typeIcons] || <Bug className="h-5 w-5" />}
                <span>错误详情</span>
              </CardTitle>
              <CardDescription>
                  ...
                  <div className="text-sm space-y-1">
                    {selectedError.context.url && (
                      <div>
                        ...
                        <span className="ml-2">{selectedError.context.url}</span>
                      </div>
                    )}
                    {selectedError.context.method && (
                      <div>
                        <span className="text-gray-500">方法:</span>
                        <span className="ml-2">{selectedError.context.method}</span>
                      </div>
                    )}
