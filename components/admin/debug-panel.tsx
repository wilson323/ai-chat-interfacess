      ...
      <Card className="border-yellow-300 bg-yellow-50 shadow-lg">
        <CardHeader className="bg-yellow-100 py-2">
          <CardTitle className="text-sm font-medium text-yellow-800 flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <Button variant="ghost" size="sm" onClick={forceRefresh} className="h-6 text-xs">
                刷新
              </Button>
              <Button variant="ghost" size="sm" onClick={toggleVisibility} className="h-6 text-xs">
                关闭
              </Button>
            </div>
          </CardTitle>
        ...
        <CardContent className="p-2 text-xs">
          ...
          <div className="mb-2">
            ...
            <pre className="bg-white p-2 rounded text-[10px] max-h-40 overflow-auto">
              {selectedAgent ? safeStringify({ ...selectedAgent, icon: "[React Element]" }, 2) : "无选中智能体"}
            </pre>
          </div>

          <div>
            <h3 className="font-bold mb-1">本地存储中的智能体</h3>
            <pre className="bg-white p-2 rounded text-[10px] max-h-40 overflow-auto">{storageData || "未加载"}</pre>
          </div>
        </CardContent>
      </Card>
    </div>
