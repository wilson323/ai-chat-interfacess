              ...
              <ResponsiveContainer width="100%" height={400}>
                <ComposedChart data={usageChartData}>
                  ...
                  {showConfidenceIntervals && (
                    <Area
                      type="monotone"
                      dataKey={(item) => item.type === 'forecast' ? item.predicted * (1 + (100 - item.confidence) / 100) : null}
                      stroke="none"
                      fill="#10b981"
                      fillOpacity={0.1}
                      name="置信区间上限"
              ...
              <ResponsiveContainer width="100%" height={400}>
                <AreaChart data={userGrowthChartData}>
                  ...
                  {showConfidenceIntervals && (
                    <Area
                      type="monotone"
                      dataKey={(item) => item.type === 'forecast' ? item.users * (1 + (100 - item.confidence) / 200) : null}
                      stroke="none"
                      fill="#3b82f6"
                      fillOpacity={0.1}
                      name="置信区间"
                ...
                <div className="space-y-4">
                  <div className="border rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-2xl font-bold text-green-600">{predictedGrowth}%</span>
                    </div>
                    <div className="text-sm text-gray-600">
                      相比上月增长 {predictedGrowth > 0 ? '+' : ''}{(predictedGrowth * 0.8).toFixed(1)}%
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <div className="font-medium">{(predictedGrowth / 30).toFixed(3)}%</div>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <div className="font-medium">{(predictedGrowth * 12).toFixed(1)}%</div>
                    </div>
                    <div>
                      <div className="text-gray-500">预计用户数</div>
                      <div className="font-medium">
                        {Math.round(data.userGrowth.predicted[data.userGrowth.predicted.length - 1]?.users || 0).toLocaleString()}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <CardContent>
                <div className="space-y-3">
                  <div className="p-3 bg-blue-50 rounded-lg">
                    ...
                    <p className="text-sm text-blue-600">简化注册步骤，提高转化率20%</p>
                  </div>
                  <div className="p-3 bg-green-50 rounded-lg">
                    <h4 className="font-medium text-green-800">口碑营销</h4>
                    <p className="text-sm text-green-600">激励用户推荐，获得高质量用户</p>
                  </div>
                  <div className="p-3 bg-purple-50 rounded-lg">
                    <h4 className="font-medium text-purple-800">内容营销</h4>
                    <p className="text-sm text-purple-600">提供有价值的内容吸引目标用户</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={400}>
                <LineChart data={resourceChartData}>
                  ...
                  <XAxis dataKey="date" tick={{ fontSize: 12 }} />
                  <YAxis domain={[0, 100]} />
                  <Tooltip formatter={(value, name) => [`${value}%`, name === 'cpu' ? 'CPU使用率' : name === 'memory' ? '内存使用率' : '存储使用率']} />
                  <Legend />
                  <Line type="monotone" dataKey="cpu" stroke="#ef4444" strokeWidth={2} name="CPU使用率" />
                  <Line type="monotone" dataKey="memory" stroke="#f59e0b" strokeWidth={2} name="内存使用率" />
                  <Line type="monotone" dataKey="storage" stroke="#8b5cf6" strokeWidth={2} name="存储使用率" />
                </LineChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
                ...
                {data.resourceForecast.recommendations.map((rec, index) => (
                  <div key={index} className="border rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Badge variant={rec.timeframe.includes('7') ? 'default' : 'secondary'}>
                        {rec.timeframe}
                      </Badge>
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card>
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-600">当前成本</p>
            ...
            <Card>
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-600">预测成本</p>
            ...
            <Card>
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-600">优化空间</p>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-3 gap-4 text-center">
                    ...
                    <div className="p-4 bg-orange-50 rounded-lg">
                      <div className="text-2xl font-bold text-orange-600">{anomalyCounts.medium}</div>
                      <div className="text-sm text-orange-600">中风险</div>
                    </div>
                    <div className="p-4 bg-yellow-50 rounded-lg">
                      <div className="text-2xl font-bold text-yellow-600">{anomalyCounts.low}</div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span>异常检测率</span>
                      <span className="font-medium">{((totalAnomalies / 1000) * 100).toFixed(2)}%</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>误报率</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 border rounded-lg">
                  ...
                  <div className="text-2xl font-bold text-red-600">
                    {data.anomalyDetection.filter(a => a.metric === '响应时间').length}
                  </div>
                ...
                <div className="p-4 border rounded-lg">
                  ...
                  <div className="text-2xl font-bold text-orange-600">
                    {data.anomalyDetection.filter(a => a.metric === 'Token使用量').length}
                  </div>
                ...
                <div className="p-4 border rounded-lg">
                  ...
                  <div className="text-2xl font-bold text-blue-600">
                    {data.anomalyDetection.filter(a => !['响应时间', 'Token使用量'].includes(a.metric)).length}
                  </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="p-4 bg-green-50 border-l-4 border-green-500 rounded">
                    <h4 className="font-medium text-green-800 mb-2">增长机会</h4>
                  ...
                  <div className="p-4 bg-orange-50 border-l-4 border-orange-500 rounded">
                    <h4 className="font-medium text-orange-800 mb-2">风险预警</h4>
                  ...
                  <div className="p-4 bg-purple-50 border-l-4 border-purple-500 rounded">
                    <h4 className="font-medium text-purple-800 mb-2">创新建议</h4>
