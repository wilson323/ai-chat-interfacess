...
export function AgentPerformanceAnalytics({
  ...
  const [_selectedAgent] = useState<string>('all');
  ...
  const avgResponseTime =
    data.responseTimeDistribution.reduce(
      (sum, agent) => sum + agent.median,
      0
    ) / data.responseTimeDistribution.length;
  const avgErrorRate =
    data.errorRates.reduce((sum, agent) => sum + agent.errorRate, 0) /
    data.errorRates.length;
  const avgSatisfaction =
    data.satisfactionAnalysis.reduce(
      (sum, agent) => sum + agent.avgSatisfaction,
      0
    ) / data.satisfactionAnalysis.length;
          ...
          <CardContent className='p-6'>
            <div className='flex items-center justify-between'>
              <div>
                ...
                <p className='text-2xl font-bold'>
                  {Math.round(
                    data.performanceRadar.reduce(
                      (sum, agent) => sum + calculatePerformanceScore(agent),
                      0
                    ) / data.performanceRadar.length
                  )}
                </p>
            ...
            <CardContent>
              <div className='space-y-3'>
                {sortedAgents.map((agent) => {
                  ...
                  const errorRate = data.errorRates.find(
                    e => e.agentId === agent.agentId
                  );
                  const satisfaction = data.satisfactionAnalysis.find(
                    ...
                    >
                      <div className='flex items-center gap-4'>
                        <div className='flex items-center justify-center w-8 h-8 bg-blue-100 text-blue-800 rounded-full font-bold'>
                          {sortedAgents.findIndex(a => a.agentId === agent.agentId) + 1}
                        </div>
                        <div>
                        ...
                        <div className='text-right'>
                          <div className='text-sm'>
                            <span className='text-green-600'>
                              可靠性: {agent.reliability}%
                            </span>
                            <span className='mx-2'>|</span>
                            <span className='text-blue-600'>
                              满意度: {satisfaction?.avgSatisfaction.toFixed(2)}
                            </span>
                          </div>
                          <div className='text-xs text-gray-500'>
                            错误率:{' '}
                            {((errorRate?.errorRate || 0) * 100).toFixed(2)}%
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className='space-y-4'>
                  {data.responseTimeDistribution.map((agent) => (
            ...
            <Card>
              ...
              <CardContent>
                <ResponsiveContainer width='100%' height={300}>
                  <BarChart
                    data={data.errorRates.map(agent => ({
                      name: agent.agentName,
                      ...
                      <div className='mt-2'>
                        ...
                        <div className='w-full bg-gray-200 rounded-full h-2 mt-1'>
                          <div
                            ...
                            style={{
                              width: `${Math.min(agent.errorRate * 1000, 100)}%`,
                            }}
                          />
                        </div>
                      </div>
          ...
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
            ...
            <Card>
              ...
              <CardContent>
                <ResponsiveContainer width='100%' height={300}>
                  <BarChart
                    data={data.satisfactionAnalysis.map(agent => ({
                      name: agent.agentName,
            ...
            <Card>
              ...
              <CardContent>
                <div className='space-y-4'>
                  {data.satisfactionAnalysis.map((agent) => (
                    <div key={agent.agentId} className='border rounded-lg p-4'>
                      <div className='flex items-center justify-between mb-3'>
                        <h4 className='font-medium'>{agent.agentName}</h4>
                        <div className='flex items-center gap-2'>
                          <span className='text-lg font-bold'>
                            {agent.avgSatisfaction.toFixed(2)}
                          </span>
                          ...
                          <div className='flex items-center gap-2'>
                            <div className='w-24 bg-gray-200 rounded-full h-2'>
                              <div
                                ...
                                style={{
                                  width: `${(agent.satisfactionDistribution.positive / (agent.satisfactionDistribution.positive + agent.satisfactionDistribution.neutral + agent.satisfactionDistribution.negative)) * 100}%`,
                                }}
                              />
                            </div>
                            <span className='text-sm font-medium'>
                              {agent.satisfactionDistribution.positive}
                            </span>
                          </div>
                        </div>
                        <div className='flex justify-between items-center'>
                          ...
                          <div className='flex items-center gap-2'>
                            <div className='w-24 bg-gray-200 rounded-full h-2'>
                              <div
                                ...
                                style={{
                                  width: `${(agent.satisfactionDistribution.neutral / (agent.satisfactionDistribution.positive + agent.satisfactionDistribution.neutral + agent.satisfactionDistribution.negative)) * 100}%`,
                                }}
                              />
                            </div>
                            <span className='text-sm font-medium'>
                              {agent.satisfactionDistribution.neutral}
                            </span>
                          </div>
                        </div>
                        <div className='flex justify-between items-center'>
                          ...
                          <div className='flex items-center gap-2'>
                            <div className='w-24 bg-gray-200 rounded-full h-2'>
                              <div
                                ...
                                style={{
                                  width: `${(agent.satisfactionDistribution.negative / (agent.satisfactionDistribution.positive + agent.satisfactionDistribution.neutral + agent.satisfactionDistribution.negative)) * 100}%`,
                                }}
                              />
                            </div>
                            <span className='text-sm font-medium'>
                              {agent.satisfactionDistribution.negative}
                            </span>
                          </div>
                        </div>
                      </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                {data.performanceRadar.map((agent) => {
                  const score = calculatePerformanceScore(agent);
                  const suggestions = [];

                  if (agent.speed < 60) suggestions.push('优化响应速度');
                  if (agent.reliability < 80)
                    suggestions.push('提高系统稳定性');
                  if (agent.satisfaction < 3.5)
                    suggestions.push('改善用户体验');
