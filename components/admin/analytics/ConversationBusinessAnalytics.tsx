...
export function ConversationBusinessAnalytics({
  ...
  const messageTypeData = [
    { name: '文本消息', value: conversationData.messageTypeDistribution.text, color: '#3b82f6' },
    { name: '图像消息', value: conversationData.messageTypeDistribution.image, color: '#10b981' },
    { name: '文件消息', value: conversationData.messageTypeDistribution.file, color: '#f59e0b' },
    { name: '语音消息', value: conversationData.messageTypeDistribution.voice, color: '#ef4444' },
    { name: '混合消息', value: conversationData.messageTypeDistribution.mixed, color: '#8b5cf6' },
  ];

  // 关键词趋势数据
  const keywordTrendData = conversationData.keywordAnalysis.slice(0, 8).map(keyword => ({
  ...
  const languagePieData = conversationData.languageDistribution.map(lang => ({
    ...
    color: lang.language === '中文' ? '#3b82f6' :
            lang.language === 'English' ? '#10b981' :
            lang.language === '日本語' ? '#f59e0b' : '#ef4444',
  }));
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>平均对话时长: {conversationData.conversationLength.avgLength}秒</CardDescription>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={conversationData.conversationLength.distribution}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="range" tick={{ fontSize: 12 }} />
                    <YAxis />
                    <Tooltip
                      formatter={(value, name) => [value, name === 'count' ? '数量' : '占比']}
                    />
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {conversationData.languageDistribution.map((lang, index) => (
                      <div key={index} className="border rounded-lg p-3">
                        <h5 className="font-medium mb-2">{lang.language}</h5>
                        <div className="space-y-1">
                          {lang.geoDistribution.map((geo, geoIndex) => (
                            <div key={geoIndex} className="flex justify-between items-center text-sm">
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {conversationData.keywordAnalysis.slice(0, 6).map((keyword, index) => (
                    <div key={index} className="border rounded-lg p-4">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="font-medium">{keyword.keyword}</h4>
                        <div className="flex items-center gap-2">
                          <span className="text-lg font-bold">{keyword.frequency}</span>
                      ...
                      <div className="text-sm text-gray-600 mb-2">
                        趋势: {keyword.trend === 'up' ? '上升' : keyword.trend === 'down' ? '下降' : '稳定'}
                      </div>
                      <div>
                        <span className="text-sm text-gray-500">相关智能体:</span>
                        <div className="flex gap-1 mt-1">
                          {keyword.relatedAgents.map((agentId, agentIndex) => (
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={costByAgentData}>
                    ...
                    <Tooltip formatter={(value, name) => [
                      ...
                      name === 'cost' ? '成本' : name === 'tokens' ? 'Token数量' : '占比'
                    ]} />
                    <Legend />
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={400}>
                <RadarChart data={valueRadarData}>
                  ...
                  {valueRadarData.map((agent, index) => (
                    <Radar
                      key={agent.subject}
                      name={agent.subject}
                      dataKey="subject"
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {businessData.valueAssessment.map((agent, index) => (
                  <div key={agent.agentId} className="border rounded-lg p-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium">{agent.businessValue}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-500">用户采用率</span>
                        <span className="font-medium">{agent.userAdoption}%</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-500">战略重要性</span>
                        <span className="font-medium">{agent.strategicImportance}</span>
                      </div>
                      <div className="pt-2 border-t">
                        <div className="flex justify-between items-center">
                          <span className="font-medium">综合评分</span>
                          <Badge variant={agent.overallScore >= 80 ? 'default' : agent.overallScore >= 60 ? 'secondary' : 'destructive'}>
                            {agent.overallScore}
                          </Badge>
                        </div>
                      </div>
        ...
        <TabsContent value="optimization" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {businessData.costAnalysis.optimizationSuggestions.map((suggestion, index) => (
                    <div key={index} className="border rounded-lg p-4">
                      <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="mt-2">
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div
                            ...
                            style={{ width: `${Math.min(suggestion.potentialSavings / totalCost * 100, 100)}%` }}
                          />
                        </div>
                        <div className="text-xs text-gray-500 mt-1">
                          占总成本的 {((suggestion.potentialSavings / totalCost) * 100).toFixed(1)}%
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="space-y-4">
                  <div className="border rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-lg font-bold">{businessData.efficiencyMetrics.avgSessionDuration}秒</span>
                    </div>
                  ...
                  <div className="border rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-lg font-bold">{businessData.efficiencyMetrics.avgResponseTime}ms</span>
                    </div>
                    <div className="text-sm text-gray-600">
                      系统响应速度，影响用户体验
                    </div>
                  ...
                  <div className="border rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-lg font-bold">{businessData.efficiencyMetrics.timeSaved}分钟</span>
                    </div>
                    <div className="text-sm text-gray-600">
                      相比传统方式节省的时间成本
                    </div>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="p-6 bg-blue-50 rounded-lg">
                  <h4 className="font-bold text-blue-800 mb-4">短期优化 (1-3个月)</h4>
                ...
                <div className="p-6 bg-green-50 rounded-lg">
                  <h4 className="font-bold text-green-800 mb-4">长期优化 (3-12个月)</h4>
