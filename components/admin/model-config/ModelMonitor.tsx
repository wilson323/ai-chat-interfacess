...
export function ModelMonitor({ modelId, timeRange = '24h' }: ModelMonitorProps) {
  ...
  const getCurrentMetrics = (model: ModelPerformanceMetrics) => {
    switch (selectedTimeRange) {
      case '24h': return model.last24Hours
      case '7d': return model.last7Days
      case '30d': return model.last30Days
      default: return model.last24Hours
    }
  }
  ...
  const getStatusIcon = (rate: number) => {
    ...
    if (rate >= 0.90) return <AlertCircle className="h-4 w-4 text-yellow-600" />
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  {metrics.reduce((sum, m) => sum + getCurrentMetrics(m).requests, 0).toLocaleString()}
                </p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  {Math.round(metrics.reduce((sum, m) => sum + getCurrentMetrics(m).avgResponseTime, 0) / metrics.length)}ms
                </p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  {Math.round(metrics.reduce((sum, m) => sum + m.successRate, 0) / metrics.length * 100)}%
                </p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  ${metrics.reduce((sum, m) => sum + getCurrentMetrics(m).cost, 0).toFixed(2)}
                </p>
              </div>
              ...
              {metrics.map((metric) => {
                ...
                return (
                  <TableRow key={metric.modelId}>
                    ...
                    <TableCell>
                      {current.requests.toLocaleString()}
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Badge 
                        variant={metric.successRate >= 0.95 ? "default" : "secondary"}
                        className={metric.successRate >= 0.95 ? "bg-green-100 text-green-800" : ""}
                      >
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {metrics.map((metric) => (
              <div key={metric.modelId} className="flex items-center justify-between p-4 border rounded-lg">
                <div>
                ...
                <div className="flex items-center gap-2">
                  <Badge variant={metric.errorRate > 0.1 ? "destructive" : "secondary"}>
