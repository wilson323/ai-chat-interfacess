      ...
      <CardContent className="flex-1 overflow-y-auto p-6">
        ...
        {migrateResult && (
          <Alert className="mb-4">
            ...
            <AlertDescription><pre className="whitespace-pre-wrap text-xs">{migrateResult}</pre></AlertDescription>
          </Alert>
        )}
                ...
                <tbody>
                  {Object.entries(fields as any).map(([field, info]: any) => (
                    <tr key={field}>
                      ...
                      <td className="p-2 border text-center">{info.primaryKey ? "是" : ""}</td>
                      <td className="p-2 border text-center">{info.allowNull ? "是" : "否"}</td>
                      <td className="p-2 border font-mono">{info.defaultValue === undefined ? "" : String(info.defaultValue)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
