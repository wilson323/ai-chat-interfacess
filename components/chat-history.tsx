...
export function ChatHistory({
  ...
  const [editingTagSessionId, setEditingTagSessionId] = useState<string | null>(
    null
  );
  ...
  const loadChatHistory = async () => {
    ...
    try {
      ...
      if (sessions.length === 0) {
        console.warn(
          'No chat sessions found in storage. This might be normal for new users.'
        );
        setDebugInfo(
          "No chat sessions found. This might be normal if you haven't had any conversations yet."
        );
        ...
        if (rebuiltSessions.length > 0) {
          ...
          setDebugInfo(null);
          console.log(
            'Successfully rebuilt chat index and found sessions:',
            rebuiltSessions.length
          );
        }
      } else {
        setDebugInfo(null);
      ...
      setDebugInfo(
        `Error loading chat history: ${error instanceof Error ? error.message : String(error)}`
      );
      toast({
        title: '加载失败',
  ...
  const handleSessionSelect = async (session: ChatSessionIndexItem) => {
    ...
    try {
      ...
      if (messages) {
        ...
        console.log(
          `Loaded ${messages.length} messages for session ${session.id}`
        );
      } else {
  ...
  const handleBatchDelete = async () => {
    ...
    setLocalSessions((prev: any[]) =>
      prev.filter(session => !selectedSessionIds.includes(session.id))
    );
    setSelectedSessionIds([]);
    toast({
      title: t('deleted'),
      description: `已批量删除${selectedSessionIds.length}条会话`,
    });
  };
  ...
  const filteredSessions = localSessions.filter(
    session =>
      ...
      (!searchQuery ||
        ...
        (session.tags && session.tags.some(tag => tag.includes(searchQuery))))
  );

  // 统一分页与本地/远端历史
  const mergedSessions = [
    ...chatSessions,
    ...localSessions.filter(s => !chatSessions.some(cs => cs.id === s.id)),
  ];
