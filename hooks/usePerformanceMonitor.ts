...
export function usePerformanceTracker() {
  const [trackedOperations, setTrackedOperations] = useState<Array<{
    name: string
    startTime: number
    endTime?: number
    duration?: number
  ...
  const startTracking = useCallback((operationName: string) => {
    ...
    setTrackedOperations(prev => [
      ...prev,
      { name: operationName, startTime }
    ])
    return startTime
  }, [])

  // 结束跟踪操作
  const endTracking = useCallback((operationName: string, startTime?: number) => {
