...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (timeRange) {
      case '1h':
        filteredData = performanceData.filter(data => now - data.timestamp < 60 * 60 * 1000)
        break
      case '6h':
        filteredData = performanceData.filter(data => now - data.timestamp < 6 * 60 * 60 * 1000)
        break
      case '24h':
        filteredData = performanceData.filter(data => now - data.timestamp < 24 * 60 * 60 * 1000)
        break
      case '7d':
        filteredData = performanceData.filter(data => now - data.timestamp < 7 * 24 * 60 * 60 * 1000)
        break
    }
    ...
    const stats = {
      ...
      summary: {
        averageMemoryUsage: filteredData.length > 0 
          ? filteredData.reduce((sum, data) => sum + data.memory.percentage, 0) / filteredData.length 
          : 0,
        averageCpuUsage: filteredData.length > 0 
          ? filteredData.reduce((sum, data) => sum + data.cpu.usage, 0) / filteredData.length 
          : 0,
        totalRequests: currentMetrics.requests.total,
        successRate: currentMetrics.requests.total > 0 
          ? (currentMetrics.requests.success / currentMetrics.requests.total) * 100 
          : 0,
