  ...
  try {
    ...
    switch (action) {
      ...
      case 'optimize':
        ...
        // 应用优化建议
        if (
          optimization.newPolicy &&
          Object.keys(optimization.newPolicy).length > 0
        ) {
      ...
      case 'set':
        const { key, value: setValue, ttl } = params;
        if (!key || setValue === undefined) {
          return NextResponse.json(
        ...
        const setResult = await redisManager.set(key, setValue, ttl);
