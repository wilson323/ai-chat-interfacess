...
import { z } from 'zod'
import type { ModelConfig, ModelConfigQuery, ModelConfigResponse } from '@/types/model-config'
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (query.type) {
      filteredModels = filteredModels.filter(model => model.type === query.type)
    }

    if (query.status) {
      filteredModels = filteredModels.filter(model => model.status === query.status)
    }

    if (query.provider) {
      filteredModels = filteredModels.filter(model => model.provider === query.provider)
    }

    if (query.category) {
      filteredModels = filteredModels.filter(model => model.metadata.category === query.category)
    }
