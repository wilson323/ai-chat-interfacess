...
const mockModelConfig = {
  ...
  version: 'gpt-4',
  status: 'active',
  capabilities: [{ type: 'text', supported: true, maxTokens: 4000 }],
  parameters: {
    temperature: 0.7,
    maxTokens: 4000,
    topP: 1,
  ...
  describe('GET /api/admin/model-config', () => {
    it('应该返回模型配置列表', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/admin/model-config'
      );
    ...
    it('应该支持搜索参数', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/admin/model-config?search=test&type=openai'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('应该支持分页参数', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/admin/model-config?page=1&limit=5'
      );
      const response = await GET(request);
      const data = await response.json();
