...
function checkRequiredVariables(
  agent: typeof mockAgentWithRequiredVariables
): boolean {
...
describe('全局变量集成功能测试', () => {
  ...
  describe('必填变量检查', () => {
    it('应该正确识别有必填变量的智能体', () => {
      const needsVariables = !checkRequiredVariables(
    ...
    it('应该在有保存值时通过检查', () => {
      ...
      localStorage.setItem(
        `agent-variables-${mockAgentWithRequiredVariables.id}`,
    ...
    it('应该在保存值不完整时要求重新填写', () => {
      ...
      localStorage.setItem(
        `agent-variables-${mockAgentWithRequiredVariables.id}`,
        JSON.stringify(incompleteValues)
      );
