...
const parseForm = (
  req: NextApiRequest
): Promise<{ fields: Fields; files: formidable.Files }> => {
  ...
  return new Promise((resolve, reject) => {
    form.parse(
      req,
      (err: Error | null, fields: Fields, files: formidable.Files) => {
        if (err) {
          reject(err);
          return;
        }
...
const convertToWav = (inputPath: string, outputPath: string): Promise<void> => {
  return new Promise((resolve, reject) => {
    ...
    const ffmpegProcess = execFile(
      ...
      (error, stdout, stderr) => {
        if (error) {
          ...
          return reject(
            new Error(`Failed to convert audio to WAV: ${error.message}`)
          );
        }
        console.log(`ffmpeg stdout: ${stdout}`);
        console.log(
          `Backend: Successfully converted ${inputPath} to ${outputPath}`
        );
        resolve();
      }
    );
    ...
    ffmpegProcess.on('close', code => {
      if (code !== 0 && code !== null) {
        // null check for already rejected/resolved cases
        // Error might have been caught by the callback already
        if (!ffmpegProcess.killed) {
          // Avoid duplicate rejection if error callback already fired
...
export default async function handler(
  ...
  try {
    ...
    if (!audioFileField) {
      ...
      return res
        .status(400)
        .json({ success: false, error: '"audio" field missing.' });
    }
    ...
    if (Array.isArray(audioFileField)) {
      if (audioFileField.length === 0) {
        ...
        return res
          .status(400)
          .json({ success: false, error: '"audio" field is empty.' });
      }
    ...
    if (!singleAudioFile || !singleAudioFile.filepath) {
      ...
      return res
        .status(400)
        .json({ success: false, error: 'No valid audio file processed.' });
    }
    ...
    console.log(
      `Backend: Received audio file: ${originalTempFilePath}, size: ${singleAudioFile.size} bytes, type: ${singleAudioFile.mimetype}`
    );
    ...
    if (mimeType && (mimeType.includes('webm') || mimeType.includes('opus'))) {
      console.log(
        `Backend: Audio type is ${mimeType}, attempting conversion to WAV.`
      );
      const tempDir = os.tmpdir();
      convertedTempWavPath = path.join(
        tempDir,
        `${path.basename(originalTempFilePath, path.extname(originalTempFilePath))}_converted.wav`
      );
      ...
      try {
        ...
        console.log(
          `Backend: Using converted WAV file for streaming: ${audioPathForStreaming}`
        );
      } catch (conversionError: any) {
        console.error(
          'Backend: Audio conversion failed.',
      ...
      console.warn(
        `Backend: Received audio file with MIME type ${mimeType}, which is not explicitly WAV. Proceeding with original file, but conversion might be needed if issues persist.`
      );
    } else {
      console.log(
        `Backend: Audio type is ${mimeType || 'unknown (likely WAV)'}, using original file for streaming.`
      );
    }

    if (!audioPathForStreaming) {
      // 安全检查
      console.error(
        'Backend: audioPathForStreaming is not defined before starting WebSocket.'
      );
      return res
        .status(500)
        .json({
          success: false,
          error: 'Internal server error: audio path not set.',
        });
    }
    ...
    const transcriptionPromise = new Promise<string>((resolve, reject) => {
      ws.on('open', () => {
        console.log(
          `Backend (Task ID: ${TASK_ID}): Connected to DashScope WebSocket.`
        );
      ...
      ws.on('message', (data: RawData /*, isBinary: boolean */) => {
        ...
        try {
          ...
          console.log(
            `Backend (Task ID: ${TASK_ID}): Received raw message: ${messageString}`
          );

          const header = message.header;
          if (header?.event === 'task-started') {
            console.log(
              `Backend (Task ID: ${TASK_ID}): DashScope task started.`
            );
            taskSuccessfullyStarted = true;
            sendAudioStream(ws, audioPathForStreaming!, TASK_ID, err => {
              // 使用 audioPathForStreaming
            ...
            console.log(
              `Backend (Task ID: ${TASK_ID}): result-generated event, text: "${text || ''}", end_time: ${endTime}`
            );
            ...
            if (text && endTime !== null && endTime !== undefined) {
              ...
              console.log(
                `Backend (Task ID: ${TASK_ID}): Final result added: "${text}"`
              );
            } else if (text) {
              console.log(
                `Backend (Task ID: ${TASK_ID}): Intermediate result ignored: "${text}"`
              );
            }
          } else if (header?.event === 'task-finished') {
            console.log(
              `Backend (Task ID: ${TASK_ID}): DashScope task finished.`
            );
            taskFinishedOrFailed = true;
            ...
            const errMsg =
              header.error_message || header.message || 'Unknown task failure';
            const errCode = header.error_code || header.status_code || 'N/A';
            console.error(
              `Backend (Task ID: ${TASK_ID}): DashScope task failed. Code: ${errCode}, Message: ${errMsg}`
            );
            transcriptionError = new Error(
              `DashScope Task Failed: ${errMsg} (Code: ${errCode})`
            );
            taskFinishedOrFailed = true;
            if (
          ...
          if (!taskFinishedOrFailed) {
            console.error(
              `Backend (Task ID: ${TASK_ID}): Error parsing message from DashScope:`,
              e.message
            );
      ...
      ws.on('error', (error: Error) => {
        ...
        console.error(
          `Backend (Task ID: ${TASK_ID}): DashScope WebSocket error:`,
          error.message
        );
      ...
      ws.on('close', (code: number, reason: Buffer) => {
        ...
        console.log(
          `Backend (Task ID: ${TASK_ID}): DashScope WebSocket closed. Code: ${code}, Reason: ${reasonString}`
        );
        if (!taskFinishedOrFailed) {
          taskFinishedOrFailed = true;
          if (transcriptionError) {
            ...
            if (
              accumulatedTranscript.trim().length > 0 &&
              !transcriptionError
            ) {
              console.warn(
                `Backend (Task ID: ${TASK_ID}): WebSocket closed with accumulated transcript but no explicit finish. Resolving with current transcript.`
              );
              resolve(accumulatedTranscript.trim());
            } else {
              reject(
                new Error(
                  `WebSocket closed unexpectedly. Code: ${code}, Reason: ${reasonString}`
                )
              );
            }
          }
    ...
    const timeoutPromise = new Promise<string>((_, reject) => {
      setTimeout(() => {
        if (!taskFinishedOrFailed) {
          taskFinishedOrFailed = true;
          console.error(
            `Backend (Task ID: ${TASK_ID}): Transcription timed out.`
          );
          if (
            ws.readyState === WebSocket.OPEN ||
            ws.readyState === WebSocket.CONNECTING
    ...
    console.error(
      'Backend: Error in /api/voice/dashscope-transcribe handler:',
      error.message
    );
    ...
    if (
      ...
      (error.code === 'ENOENT' ||
        (error.spawnargs && error.spawnargs[0] === 'ffmpeg'))
    ) {
      return res
        .status(500)
        .json({
          success: false,
          error:
            'Internal server error: ffmpeg command not found. Please ensure ffmpeg is installed and in PATH.',
        });
    }
    res
      .status(500)
    ...
    const cleanupFile = (
      ...
      if (filePath) {
        fs.unlink(filePath, err => {
          if (err) {
            console.warn(
              `Backend: Failed to delete temporary ${fileNameForLog} file ${filePath}:`,
              err?.message || err
            );
          } else {
            console.log(
              `Backend: Deleted temporary ${fileNameForLog} file ${filePath}`
            );
          }
        });
      }
    ...
    if (convertedTempWavPath && convertedTempWavPath !== originalTempFilePath) {
      // 确保不重复删除
...
function sendAudioStream(
  ...
  console.log(
    `Backend (Task ID: ${taskId}): Starting audio stream from ${filePath} with chunk size ${chunkSize}`
  );
  ...
  stream.on('data', (chunk: Buffer | string) => {
    ...
    if (ws.readyState === WebSocket.OPEN) {
      if (Buffer.isBuffer(chunk)) {
        console.log(
          `Backend (Task ID: ${taskId}): Sending audio chunk, size: ${chunk.length} bytes`
        );
        ws.send(chunk);
      ...
      console.warn(
        `Backend (Task ID: ${taskId}): WebSocket not open while trying to send audio chunk. Stopping stream.`
      );
      closeStreamAndNotifyError(
        new Error('WebSocket closed during audio streaming.')
      );
    }
  });
  ...
  stream.on('end', () => {
    ...
    if (ws.readyState === WebSocket.OPEN) {
      ...
      console.log(
        `Backend (Task ID: ${taskId}): Sent finish-task to DashScope.`
      );
    } else {
      console.warn(
        `Backend (Task ID: ${taskId}): WebSocket not open when trying to send finish-task.`
      );
  ...
  stream.on('error', (err: Error) => {
    console.error(
      `Backend (Task ID: ${taskId}): Error reading audio file stream:`,
      err.message
    );
    closeStreamAndNotifyError(
      new Error(`Error reading audio file: ${err.message}`)
    );
  });
  ...
  const onWsCloseEarly = () => {
    if (!streamClosedOrErrored) {
      console.log(
        `Backend (Task ID: ${taskId}): WebSocket closed prematurely during audio streaming. Destroying stream.`
      );
      // The main promise rejection will be handled by the ws.on('close') in the handler
      closeStreamAndNotifyError(
        new Error('WebSocket closed prematurely during audio streaming.')
      ); // This calls onError
    }
  ...
  stream.on('close', () => {
    // 'close' is emitted after 'end' or 'error'
