...
class RuleChecker {
  ...
  checkCustomCodeRatio(): void {
    try {
      ...
      const customCodeLines = this.countLinesInDirectory(customComponentsPath, [
        '.tsx',
        '.ts',
      ]);
  ...
  checkTestCoverage(): void {
    try {
      // 检查是否有测试文件
      const testFiles = this.getAllFiles(join(this.projectRoot, '__tests__'), [
        '.test.ts',
        '.test.tsx',
      ]);
      const componentFiles = this.getAllFiles(
        join(this.projectRoot, 'components'),
        ['.tsx']
      );
      ...
      if (testRatio >= 80) {
        this.addResult(
          '测试覆盖率',
          'PASS',
          `测试文件覆盖率: ${testRatio.toFixed(2)}% (≥ 80%)`
        );
      } else {
        this.addResult(
          '测试覆盖率',
