...
class EnvironmentChecker {
  ...
  checkConfigurationFiles(): void {
    ...
    if (fs.existsSync('.env')) {
      ...
      this.addResult('配置文件', 'warning', '⚠️ .env 文件不存在，请复制 env.template 为 .env')
    }
  }
  ...
  async checkRedisConfiguration(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        this.addResult('Redis连接', 'warning', `⚠️ Redis连接测试跳过: ${error}`)
      }
    } catch (error) {
  ...
  checkApplicationConfiguration(): void {
    ...
    for (const feature of featureChecks) {
      ...
      const message = feature.enabled ? `✓ ${feature.name} 已启用` : `⚠️ ${feature.name} 已禁用`
  ...
  generateReport(): string {
    ...
    const totalWarning = this.results.filter(r => r.status === 'warning').length
