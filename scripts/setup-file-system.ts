    ...
    for (const dir of this.directories) {
      try {
        ...
        if (hasPermissions) {
          ...
          console.log(
            `  ❌ ${dir.path} (权限: ${currentPermissions}) - 权限不足`
          );
          invalid++;
        }
      } catch (error) {
  ...
  async validateFileSystem(): Promise<boolean> {
    ...
    for (const dir of this.directories) {
      ...
      const hasPermissions = exists
        ? this.checkDirectoryPermissions(dir.path)
        : false;
  ...
  generateReport(): string {
    ...
    for (const dir of this.directories) {
      ...
      const permissions = exists
        ? this.getDirectoryPermissions(dir.path)
        : 'N/A';
      const status = exists ? '✅' : '❌';
    ...
    report.push(
      `- 允许的文件类型: ${appConfig.features.allowedFileTypes.join(', ')}`
    );
