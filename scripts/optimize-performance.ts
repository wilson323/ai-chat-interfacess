...
interface PerformanceIssue {
  file: string
  line: number
  type: 'bundle-size' | 'runtime-performance' | 'memory-usage' | 'network-requests'
  ...
  private async checkDatabaseQueries(): Promise<void> {
    ...
    for (const file of tsFiles) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        // 检查缺少索引的查询
        if (line.includes('WHERE') && line.includes('LIKE') && line.includes('%')) {
          this.addIssue({
            file: path.relative(this.projectRoot, file),
            line: i + 1,
            type: 'runtime-performance',
  ...
  private generateReport(): void {
    ...
    const report = {
      ...
      summary: {
        ...
        runtimePerformance: this.issues.filter(i => i.type === 'runtime-performance').length,
        memoryUsage: this.issues.filter(i => i.type === 'memory-usage').length,
        networkRequests: this.issues.filter(i => i.type === 'network-requests').length
      }
    }
