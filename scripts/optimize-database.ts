...
import { getDatabaseMonitor, startDatabaseMonitoring } from '../lib/db/monitoring'
import { getQueryOptimizer, generateIndexSuggestions } from '../lib/db/query-optimizer'
...
class DatabaseOptimizer {
  ...
  private async startMonitoring(): Promise<void> {
    ...
    try {
      startDatabaseMonitoring(this.sequelize, this.redis, this.options.monitoringInterval)
  ...
  async generateReport(): Promise<string> {
    ...
    try {
      ...
      report.push(`- 平均查询时间: ${metrics.queryPerformance.averageQueryTime.toFixed(2)}ms`)
