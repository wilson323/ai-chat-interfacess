...
class ScriptSafetyChecker {
  ...
  private readonly forbiddenOperations = {
    ...
    create: ['mkdirSync', 'mkdir', 'mkdirAsync', 'fs.mkdir', 'fs.mkdirSync'],
    execute: [
      'execSync',
      'exec',
      'execAsync',
  ...
  private checkScriptForViolations(scriptPath: string): Violation[] {
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        // 检查各种违规操作
        for (const [type, operations] of Object.entries(
  ...
  private calculateSeverity(
    ...
    const hasMediumSeverity = violations.some(v =>
      mediumSeverityTypes.includes(v.type)
    );
  ...
  private generateRecommendations(): void {
    ...
    const highSeverityViolations = this.report.violations.filter(
      v => v.severity === 'high'
    );
  ...
  private calculateSafetyScore(): number {
    ...
    // 高危违规扣分
    const highSeverityCount = this.report.violations.filter(
      v => v.severity === 'high'
    ).length;
    score -= highSeverityCount * 20;

    // 中危违规扣分
    const mediumSeverityCount = this.report.violations.filter(
      v => v.severity === 'medium'
    ).length;
