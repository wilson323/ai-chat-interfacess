...
class CodeStandardChecker {
    ...
    for (const file of tsFiles) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('import')) {
          ...
          if (lastImportType && this.getImportPriority(importType) < this.getImportPriority(lastImportType)) {
            this.addIssue({
              file: path.relative(this.projectRoot, file),
              line: i + 1,
              column: 1,
              ...
              severity: 'warning',
              message: '导入顺序不符合规范',
              suggestion: '按以下顺序排列导入：1. 第三方库 2. 内部模块 3. 相对路径'
            })
          }
  ...
  private async checkReactPatterns(): Promise<void> {
    ...
    for (const file of tsxFiles) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        // 检查useEffect依赖
        if (line.includes('useEffect') && !line.includes('[]') && !line.includes('[deps]')) {
          this.addIssue({
            file: path.relative(this.projectRoot, file),
            line: i + 1,
            column: 1,
  ...
  private getImportType(line: string): string {
    ...
    if (line.includes('from \'./') || line.includes('from \'../')) return 'relative'
    return 'external'
  }
  ...
  private generateReport(): void {
    const reportPath = path.join(this.projectRoot, 'code-standards-report.json')
    const report = {
      ...
      summary: {
        ...
        suggestions: this.issues.filter(i => i.severity === 'suggestion').length
      }
    }
    ...
    if (this.issues.length > 0) {
      ...
      this.issues.forEach((issue, index) => {
        console.log(`${index + 1}. [${issue.severity.toUpperCase()}] ${issue.file}:${issue.line}`)
