...
function checkEnvironmentConfig() {
  ...
  try {
    ...
    console.log(
      `  Rate Limit: ${appConfig.api.rateLimit.max} requests per ${appConfig.api.rateLimit.windowMs / 1000}s`
    );
    ...
    console.log(
      `  File Upload: ${appConfig.features.enableFileUpload ? '✅' : '❌'}`
    );
    console.log(
      `  Image Upload: ${appConfig.features.enableImageUpload ? '✅' : '❌'}`
    );
    console.log(
      `  Streaming: ${appConfig.features.enableStreaming ? '✅' : '❌'}`
    );
    console.log(
      `  Max File Size: ${(appConfig.features.maxFileSize / 1024 / 1024).toFixed(2)}MB`
    );
    console.log(
      `  Allowed Types: ${appConfig.features.allowedFileTypes.join(', ')}`
    );

    // 安全配置
    console.log('\n🔒 安全配置:');
    console.log(
      `  JWT Secret: ${appConfig.security.jwtSecret ? '✅ 已设置' : '❌ 未设置'}`
    );
    ...
    console.log(
      `  Rate Limit: ${appConfig.security.rateLimitEnabled ? '✅' : '❌'}`
    );
    ...
    console.log(
      `  Password: ${appConfig.redis.password ? '✅ 已设置' : '❌ 未设置'}`
    );
