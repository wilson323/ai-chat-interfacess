...
class TestPerformanceMonitor {
  ...
  endTest(testName: string) {
    ...
    if (duration > 1000) {
      console.warn(
        `Slow test detected: ${testName} (${duration.toFixed(2)}ms)`
      );
    }
  }
  ...
  checkPerformanceAlerts(): Array<{
    ...
    if (report.memoryUsage.peak > 100 * 1024 * 1024) {
      ...
      alerts.push({
        ...
        severity:
          report.memoryUsage.peak > 500 * 1024 * 1024 ? 'critical' : 'warning',
    ...
    // 高CPU使用告警
    if (report.cpuUsage.peak > 1000000) {
      // 1秒
      alerts.push({
        type: 'high_cpu',
...
## 慢测试详情
${report.slowTestDetails
  .map(test => `- ${test.testName}: ${test.duration.toFixed(2)}ms`)
  .join('\n')}
`;
...
export const performanceHelpers = {
  ...
  // 测量同步函数执行时间
  measureSyncFunction: <T>(fn: () => T, name: string): T => {
    testPerformanceMonitor.startTest(name);
