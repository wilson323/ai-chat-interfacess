...
class DatabaseQueryOptimizer {
  ...
  async analyzeQuery(query: string, params: any[] = []): Promise<QueryAnalysis> {
    ...
    try {
      // 执行查询并获取执行计划
      const explainResult = await this.sequelize.query(`EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON) ${query}`, {
   ...
   * 分析执行计划
   */
  private analyzeExecutionPlan(query: string, plan: any, executionTime: number): QueryAnalysis {
    ...
    const analyzeNode = (node: any) => {
      if (node['Node Type']) {
        ...
        if (nodeType === 'Seq Scan') {
          ...
          score -= 20
        } else if (nodeType === 'Index Scan' || nodeType === 'Index Only Scan') {
          if (node['Index Name']) {
  ...
  async generateIndexSuggestions(): Promise<IndexSuggestion[]> {
    ...
    try {
      ...
      for (const table of tables as any[]) {
        ...
        // 生成索引建议
        const tableSuggestions = this.analyzeTableForIndexes(tableName, columns, indexes)
  ...
  private analyzeTableForIndexes(
    ...
    // 分析外键
    const foreignKeys = columns.filter((col: any) => 
    ...
    // 分析时间字段
    const timeFields = columns.filter((col: any) => 
    ...
    // 分析状态字段
    const statusFields = columns.filter((col: any) => 
 ...
 * 获取查询优化器实例
 */
export function getQueryOptimizer(sequelize: Sequelize): DatabaseQueryOptimizer {
  if (!globalOptimizer) {
 ...
 * 分析查询性能
 */
export async function analyzeQuery(query: string, params: any[] = [], sequelize: Sequelize): Promise<QueryAnalysis> {
 ...
 * 生成索引建议
 */
export async function generateIndexSuggestions(sequelize: Sequelize): Promise<IndexSuggestion[]> {
  const optimizer = getQueryOptimizer(sequelize)
 ...
 * 优化查询
 */
export async function optimizeQuery(query: string, params: any[] = [], sequelize: Sequelize): Promise<QueryOptimization> {
  const optimizer = getQueryOptimizer(sequelize)
