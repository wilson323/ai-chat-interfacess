  ...
  private async collectQueryPerformanceMetrics(): Promise<void> {
    try {
      ...
      if (this.queryHistory.length > 0) {
        const totalTime = this.queryHistory.reduce(
          (sum, q) => sum + q.executionTime,
          0
        );
        this.metrics.queryPerformance.averageQueryTime =
          totalTime / this.queryHistory.length;
      }

      // 计算慢查询数（超过100ms）
      this.metrics.queryPerformance.slowQueries = this.queryHistory.filter(
        q => q.executionTime > 100
      ).length;

      // 计算失败查询数
      this.metrics.queryPerformance.failedQueries = this.queryHistory.filter(
        q => !q.success
      ).length;
  ...
  private async collectResourceUsageMetrics(): Promise<void> {
    try {
      ...
      this.metrics.resourceUsage.cpuUsage =
        (cpuUsage.user + cpuUsage.system) / 1000000; // 秒
  ...
  checkHealth(): {
    ...
    const errorRate =
      queryPerf.totalQueries > 0
        ? queryPerf.failedQueries / queryPerf.totalQueries
        : 0;
    if (errorRate > 0.05) {
      issues.push('查询错误率过高');
      score -= 25;
    ...
    // 检查内存使用
    if (this.metrics.resourceUsage.memoryUsage > 1000) {
      // 1GB
      issues.push('内存使用量过高');
      score -= 10;
    }
 ...
 * 检查数据库健康状态
 */
export function checkDatabaseHealth(): ReturnType<
