...
class DatabaseBackupManager {
   ...
   * 执行备份命令
   */
  private async executeBackupCommand(command: string, backupInfo: BackupInfo): Promise<void> {
  ...
  async restoreDatabase(options: RestoreOptions): Promise<void> {
    ...
    try {
      ...
      if (options.createDatabase) {
        await this.createDatabase(options.targetDatabase || this.config.database)
      }
      ...
      // 执行恢复
      await this.executeRestoreCommand(backup.path, options.targetDatabase || this.config.database)
   ...
   * 执行恢复命令
   */
  private async executeRestoreCommand(backupPath: string, databaseName: string): Promise<void> {
 ...
 * 获取备份管理器实例
 */
export function getBackupManager(config?: Partial<BackupConfig>): DatabaseBackupManager {
 ...
 * 创建全量备份
 */
export async function createFullBackup(config?: Partial<BackupConfig>): Promise<BackupInfo> {
 ...
 * 创建增量备份
 */
export async function createIncrementalBackup(config?: Partial<BackupConfig>): Promise<BackupInfo> {
  const manager = getBackupManager(config)
 ...
 * 恢复数据库
 */
export async function restoreDatabase(options: RestoreOptions, config?: Partial<BackupConfig>): Promise<void> {
  const manager = getBackupManager(config)
 ...
 * 清理过期备份
 */
export async function cleanupExpiredBackups(config?: Partial<BackupConfig>): Promise<void> {
  const manager = getBackupManager(config)
