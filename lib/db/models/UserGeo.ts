...
export interface UserGeoCreationAttributes extends Optional<UserGeoAttributes, 'id' | 'usageCount' | 'lastSeenAt'> {}

export class UserGeo extends Model<UserGeoAttributes, UserGeoCreationAttributes> implements UserGeoAttributes {
  public id!: number;
...
// 静态方法
UserGeo.createOrUpdate = async function(
  geoData: {
    ipAddress: string;
    country: string;
    region?: string;
...
UserGeo.getTopLocations = async function(
  limit: number = 10,
  timeRange?: { start: Date; end: Date }
): Promise<Array<{ id: number; country: string; city: string; usageCount: number }>> {
  const where: any = {};
  if (timeRange) {
    where.lastSeenAt = {
...
UserGeo.getLocationStats = async function(
  params: {
    startDate?: Date;
    endDate?: Date;
  ...
  const where: any = {};
  if (startDate) where.lastSeenAt = { [sequelize.Op.gte]: startDate };
  if (endDate) where.lastSeenAt = { ...where.lastSeenAt, [sequelize.Op.lte]: endDate };
...
UserGeo.getCleanupCandidates = async function(daysToKeep: number = 365): Promise<UserGeo[]> {
...
UserGeo.cleanupOldData = async function(daysToKeep: number = 365): Promise<number> {
  const cutoffDate = new Date();
  cutoffDate.setDate(cutoffDate.getDate() - daysToKeep);
