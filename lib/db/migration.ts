...
class DatabaseMigrationManager {
   ...
   * 解析迁移文件
   */
  private parseMigrationFile(fileName: string, content: string): MigrationInfo | null {
    try {
      // 提取版本号
   ...
   * 执行迁移SQL
   */
  private async executeMigration(migration: MigrationInfo, direction: 'up' | 'down'): Promise<void> {
   ...
   * 记录迁移
   */
  private async recordMigration(migration: MigrationInfo, status: string, error?: any): Promise<void> {
    const sql = `
 ...
 * 获取迁移管理器实例
 */
export function getMigrationManager(sequelize: Sequelize, config?: Partial<MigrationConfig>): DatabaseMigrationManager {
 ...
 * 执行迁移
 */
export async function runMigrations(sequelize: Sequelize, config?: Partial<MigrationConfig>): Promise<MigrationInfo[]> {
 ...
 * 回滚迁移
 */
export async function rollbackMigrations(migrations: MigrationInfo[], sequelize: Sequelize, config?: Partial<MigrationConfig>): Promise<void> {
  const manager = getMigrationManager(sequelize, config)
 ...
 * 创建迁移文件
 */
export async function createMigration(name: string, description: string, sequelize: Sequelize, config?: Partial<MigrationConfig>): Promise<string> {
  const manager = getMigrationManager(sequelize, config)
