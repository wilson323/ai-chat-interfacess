...
export interface CacheStrategy {
  ...
  warmup<T>(items: Array<{ key: string; value: T; ttl?: number }>): Promise<boolean>
}
...
export class SmartCacheStrategy implements CacheStrategy {
  ...
  private async updateAccessPattern(key: string): Promise<void> {
    ...
    if (pattern) {
      ...
      pattern.accessCount++
      pattern.averageInterval = (pattern.averageInterval * (pattern.accessCount - 1) + interval) / pattern.accessCount
...
// 创建默认策略管理器
export const cacheStrategyManager = new CacheStrategyManager(defaultCachePolicy)
