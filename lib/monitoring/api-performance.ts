...
class APIPerformanceMonitor {
  ...
  getAPIStats(): APIPerformanceStats {
    ...
    const endpointStats = this.metrics.reduce(
      ...
      {} as Record<
        string,
        { endpoint: string; duration: number; count: number }
      >
    );
    ...
    // 状态码分布
    const statusCodeDistribution = this.metrics.reduce(
      (acc, m) => {
    ...
    // 请求和响应大小统计
    const requestSizes = this.metrics
      .filter(m => m.requestSize)
      .map(m => m.requestSize!);
    const responseSizes = this.metrics
    ...
    return {
      ...
      averageRequestSize:
        requestSizes.length > 0
          ? requestSizes.reduce((sum, size) => sum + size, 0) /
            requestSizes.length
          : 0,
      averageResponseSize:
    ...
    return {
      ...
      averageDuration:
        durations.length > 0
          ...
          : 0,
      errorRate:
        filteredMetrics.length > 0 ? errorCount / filteredMetrics.length : 0,
  ...
  getErrorStats(): {
    ...
    const errorByEndpoint = errorMetrics.reduce(
      (acc, m) => {
        const key = `${m.method} ${m.endpoint}`;
        acc[key] = (acc[key] || 0) + 1;
        return acc;
    ...
    const errorByStatusCode = errorMetrics.reduce(
      (acc, m) => {
        acc[m.statusCode] = (acc[m.statusCode] || 0) + 1;
        return acc;
      },
    ...
    return {
      ...
      errorRate:
        this.metrics.length > 0 ? errorMetrics.length / this.metrics.length : 0,
      errorByEndpoint,
      errorByStatusCode,
  ...
  getPerformanceTrend(hours: number = 24): Array<{
    ...
    for (let time = startTime; time < now; time += interval) {
      ...
      const periodMetrics = this.metrics.filter(
        m => m.timestamp >= time && m.timestamp < nextTime
      );

      const totalDuration = periodMetrics.reduce(
      ...
      trend.push({
        ...
        averageDuration:
          periodMetrics.length > 0 ? totalDuration / periodMetrics.length : 0,
        errorRate:
          periodMetrics.length > 0 ? errorCount / periodMetrics.length : 0,
      });
    }
