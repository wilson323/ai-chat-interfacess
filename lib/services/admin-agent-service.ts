...
export async function createAgent(agent: Partial<Agent>): Promise<Agent> {
  ...
  const result = await res.json();
  if (!res.ok || !result.success) throw new Error(result.error || "新增智能体失败");
  return result.data;
}
...
export async function updateAgent(agent: Partial<Agent>): Promise<Agent> {
  ...
  const result = await res.json();
  if (!res.ok || !result.success) throw new Error(result.error || "更新智能体失败");
  return result.data;
}
...
export async function deleteAgent(id: string): Promise<void> {
  ...
  const result = await res.json();
  if (!res.ok || !result.success) throw new Error(result.error || "删除智能体失败");
} 