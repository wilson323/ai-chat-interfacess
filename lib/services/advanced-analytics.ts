import { AgentUsage, ChatSession, User, UserGeo, AgentConfig } from '@/lib/db/models';
import { Op, fn, col, literal } from 'sequelize';
import { subDays, startOfDay, endOfDay, format, startOfWeek, endOfWeek, startOfMonth, endOfMonth } from 'date-fns';
...
export interface PredictionData {
  ...
  resourceForecast: {
    predictedLoad: Array<{ date: string; cpu: number; memory: number; storage: number }>;
...
export class AdvancedAnalyticsService {
  ...
  static async getUserBehaviorAnalytics(
    ...
    const [hourlyActivity, userRetention, userPaths, userSegments] = await Promise.all([
  ...
  private static async getHourlyActivity(startDate: Date, endDate: Date) {
    const activity = await AgentUsage.findAll({
      where: {
        startTime: {
    ...
    activity.forEach((item: any) => {
      result[item.hour].count = parseInt(item.count);
    });
  ...
  private static async getUserRetention(startDate: Date, endDate: Date) {
    ...
    for (let i = 0; i < days; i++) {
      ...
      // 新用户：当天首次使用
      const newUsers = await AgentUsage.count({
        where: {
          startTime: {
      ...
      // 活跃用户：当天有使用记录
      const activeUsers = await AgentUsage.count({
        where: {
          startTime: {
            [Op.between]: [currentDate, nextDate],
          },
      ...
      // 流失用户：7天未活跃
      const churnDate = new Date(currentDate.getTime() - 7 * 24 * 60 * 60 * 1000);
  ...
  private static async getUserPaths(startDate: Date, endDate: Date) {
    const sessions = await AgentUsage.findAll({
      where: {
        startTime: {
          [Op.between]: [startDate, endDate],
        },
    ...
    return Array.from(userPaths.entries()).map(([userId, agentSequence]) => ({
      userId,
      agentSequence: [...new Set(agentSequence)], // 去重
  ...
  private static calculateConversionRates(sequence: number[]): number[] {
    ...
    for (let i = 1; i < sequence.length; i++) {
      const fromAgent = sequence[i - 1];
      const toAgent = sequence[i];
      // 简化的转换率计算，实际应该基于历史数据
  ...
  private static async getUserSegments(startDate: Date, endDate: Date) {
    const userStats = await AgentUsage.findAll({
      where: {
        startTime: {
          [Op.between]: [startDate, endDate],
        },
    ...
    const segments = {
      '高频用户': { users: [], totalUsage: 0, totalSatisfaction: 0 },
      '中频用户': { users: [], totalUsage: 0, totalSatisfaction: 0 },
      '低频用户': { users: [], totalUsage: 0, totalSatisfaction: 0 },
    } as any;

    userStats.forEach((stat: any) => {
      const sessionCount = parseInt(stat.sessionCount);
      const duration = parseInt(stat.totalDuration) || 0;
      const satisfaction = parseFloat(stat.avgSatisfaction) || 0;

      let segment = '低频用户';
      if (sessionCount > 10 || duration > 3600) segment = '高频用户';
      else if (sessionCount > 3 || duration > 600) segment = '中频用户';

      segments[segment].users.push(stat.userId);
      segments[segment].totalUsage += duration;
      segments[segment].totalSatisfaction += satisfaction;
    });

    return Object.entries(segments).map(([segment, data]: [string, any]) => ({
      segment,
      userCount: data.users.length,
      avgUsage: data.totalUsage / data.users.length || 0,
      avgSatisfaction: (data.totalSatisfaction / data.users.length || 0).toFixed(2),
    }));
  }
  ...
  static async getAgentPerformanceAnalytics(
    ...
    const [responseTimeDistribution, errorRates, satisfactionAnalysis, performanceRadar] =
      await Promise.all([
  ...
  private static async getResponseTimeDistribution(startDate: Date, endDate: Date) {
    ...
    return agents.map(agent => {
      const responseTimes = agent.usages?.map((usage: any) => usage.responseTime).filter(Boolean) || [];
      ...
      const iqr = q3 - q1;
      const outliers = sorted.filter(t => t < q1 - 1.5 * iqr || t > q3 + 1.5 * iqr).length;
  ...
  private static async getErrorRates(startDate: Date, endDate: Date) {
    ...
    return agents.map(agent => {
      const sessions = agent.usages || [];
      const totalSessions = sessions.length;
      const errorSessions = sessions.filter((s: any) => !s.isCompleted).length;
      const errorRate = totalSessions > 0 ? errorSessions / totalSessions : 0;

      // 简单的趋势分析
      const recentErrorRate = sessions
        .filter((s: any) => new Date(s.startTime) > new Date(endDate.getTime() - 7 * 24 * 60 * 60 * 1000))
        .filter((s: any) => !s.isCompleted).length /
        sessions.filter((s: any) => new Date(s.startTime) > new Date(endDate.getTime() - 7 * 24 * 60 * 60 * 1000)).length || 0;

      const trend = recentErrorRate > errorRate * 1.1 ? 'up' :
                   recentErrorRate < errorRate * 0.9 ? 'down' : 'stable';
  ...
  private static async getSatisfactionAnalysis(startDate: Date, endDate: Date) {
    ...
    return agents.map(agent => {
      const satisfactions = agent.usages?.map((u: any) => u.userSatisfaction).filter(Boolean) || [];
      ...
      const distribution = {
        positive: satisfactions.filter(s => s === 'positive').length,
        neutral: satisfactions.filter(s => s === 'neutral').length,
        negative: satisfactions.filter(s => s === 'negative').length,
      };
      ...
      // 趋势分析
      const recentSatisfactions = satisfactions.slice(-Math.floor(satisfactions.length / 3));
      const recentAvg = (
        recentSatisfactions.filter(s => s === 'positive').length * 3 +
        recentSatisfactions.filter(s => s === 'neutral').length * 2 +
        recentSatisfactions.filter(s => s === 'negative').length * 1
      ) / recentSatisfactions.length;

      const trend = recentAvg > avgSatisfaction * 1.05 ? 'improving' :
                   recentAvg < avgSatisfaction * 0.95 ? 'declining' : 'stable';
  ...
  private static async getPerformanceRadar(startDate: Date, endDate: Date) {
    ...
    return agents.map(agent => {
      ...
      // 速度 (0-100, 响应时间越短分数越高)
      const avgResponseTime = sessions.reduce((sum, s: any) => sum + (s.responseTime || 0), 0) / sessions.length || 0;
      ...
      const reliability = sessions.length > 0 ? (completedSessions / sessions.length) * 100 : 0;

      // 满意度 (0-100)
      const satisfactions = sessions.map((s: any) => s.userSatisfaction).filter(Boolean);
      const satisfaction = satisfactions.length > 0 ?
        (satisfactions.filter(s => s === 'positive').length * 100 +
         satisfactions.filter(s => s === 'neutral').length * 50) / satisfactions.length : 0;

      // 效率 (0-100, 基于token使用效率)
      const totalTokens = sessions.reduce((sum, s: any) => sum + (s.tokenUsage || 0), 0);
      const totalMessages = sessions.reduce((sum, s: any) => sum + s.messageCount, 0);
      const efficiency = totalMessages > 0 ? Math.max(0, Math.min(100, 100 - (totalTokens / totalMessages / 100))) : 0;

      // 受欢迎程度 (0-100, 使用频率)
      const maxUsage = Math.max(...agents.map(a => a.usages?.length || 0));
  ...
  static async getConversationAnalytics(
    ...
    const [messageTypeDistribution, conversationLength, keywordAnalysis, languageDistribution] =
      await Promise.all([
  ...
  private static async getMessageTypeDistribution(startDate: Date, endDate: Date) {
    const distribution = await AgentUsage.findAll({
      where: {
        startTime: {
          [Op.between]: [startDate, endDate],
        },
    ...
    distribution.forEach((item: any) => {
      result[item.messageType] = parseInt(item.count);
      result.total += parseInt(item.totalMessages || item.count);
    });
  ...
  private static async getConversationLength(startDate: Date, endDate: Date) {
    ...
    const avgLength = durations.length > 0 ? durations.reduce((a, b) => a + b, 0) / durations.length : 0;
  ...
  private static async getKeywordAnalysis(startDate: Date, endDate: Date) {
    ...
    return commonKeywords.map(keyword => ({
      ...
      relatedAgents: Array.from({ length: Math.floor(Math.random() * 3) + 1 }, () => Math.floor(Math.random() * 5) + 1),
    })).sort((a, b) => b.frequency - a.frequency).slice(0, 10);
  }

  private static async getLanguageDistribution(startDate: Date, endDate: Date) {
    const languages = [
  ...
  private static async getCostAnalysis(startDate: Date, endDate: Date) {
    const usage = await AgentUsage.findAll({
      where: {
        startTime: {
          [Op.between]: [startDate, endDate],
        },
    ...
    const totalTokens = usage.reduce((sum, u: any) => sum + (u.tokenUsage || 0), 0);
    const tokenPrice = 0.002; // $0.002 per 1K tokens
    const estimatedCost = (totalTokens / 1000) * tokenPrice;

    // 按智能体分组
    const agentCosts = new Map<number, { tokens: number; cost: number; name: string }>();
    usage.forEach((u: any) => {
    ...
    const costByAgent = Array.from(agentCosts.entries()).map(([agentId, data]) => ({
      agentId,
      agentName: data.name,
      tokens: data.tokens,
      cost: data.cost,
    ...
    const optimizationSuggestions = [
      ...
      {
        type: '提示词优化',
        potentialSavings: estimatedCost * 0.20,
        description: '优化提示词长度和结构可减少20%的token消耗',
      },
      {
        type: '智能路由',
        potentialSavings: estimatedCost * 0.10,
  ...
  private static async getRoiAnalysis(startDate: Date, endDate: Date) {
    ...
    const roiByAgent = agents.map(agent => {
      const sessions = agent.usages?.length || 0;
      const investment = 10000; // 假设每个智能体投资10000美元
      const value = sessions * estimatedValuePerSession;
      const agentRoi = investment > 0 ? ((value - investment) / investment) * 100 : 0;
  ...
  private static async getEfficiencyMetrics(startDate: Date, endDate: Date) {
    ...
    const avgSessionDuration = sessions.length > 0 ?
      sessions.reduce((sum, s: any) => sum + (s.duration || 0), 0) / sessions.length : 0;

    const avgResponseTime = sessions.length > 0 ?
      sessions.reduce((sum, s: any) => sum + (s.responseTime || 0), 0) / sessions.length : 0;
  ...
  private static async getValueAssessment(startDate: Date, endDate: Date) {
    ...
    return agents.map(agent => {
      ...
      const satisfaction = sessions.filter((s: any) => s.userSatisfaction === 'positive').length / sessions.length || 0;
      const completion = sessions.filter((s: any) => s.isCompleted).length / sessions.length || 0;

      // 计算各个维度的分数
      const businessValue = Math.min(100, usage * 10); // 基于使用频率
      const userAdoption = satisfaction * 100; // 用户满意度
      const strategicImportance = agent.type === 'core' ? 90 : agent.type === 'support' ? 70 : 50;
      const overallScore = (businessValue + userAdoption + strategicImportance) / 3;
  ...
  private static async getUsageTrend(startDate: Date, endDate: Date) {
    // 获取历史数据
    const historical = await AgentUsage.findAll({
      where: {
        startTime: {
          [Op.between]: [
    ...
    const historicalData = historical.map((h: any) => ({
      date: h.date,
      actual: parseInt(h.count),
      predicted: parseInt(h.count), // 简化：历史预测值等于实际值
    }));
  ...
  private static async getUserGrowth(startDate: Date, endDate: Date) {
    ...
    const growthRate = days > 1 ? Math.pow(totalUsers / historicalData[0].users, 1 / days) - 1 : 0;
  ...
  private static async getResourceForecast(startDate: Date, endDate: Date) {
    ...
    for (let i = 0; i < usageTrend.forecast.length; i++) {
      ...
      const load = parseInt(forecast.predicted);
  ...
  private static async getAnomalyDetection(startDate: Date, endDate: Date) {
    ...
    // 检测响应时间异常
    const responseTimes = sessions.map((s: any) => s.responseTime).filter(Boolean);
    ...
    sessions.forEach((session: any) => {
      if (session.responseTime && Math.abs(session.responseTime - avgResponseTime) > 2 * stdResponseTime) {
        anomalies.push({
          timestamp: session.startTime,
          metric: '响应时间',
          value: session.responseTime,
          expected: Math.round(avgResponseTime),
          severity: session.responseTime > avgResponseTime + 3 * stdResponseTime ? 'high' : 'medium',
    ...
    // 检测Token使用异常
    const tokenUsages = sessions.map((s: any) => s.tokenUsage).filter(Boolean);
    const avgTokenUsage = tokenUsages.reduce((a, b) => a + b, 0) / tokenUsages.length;
    const stdTokenUsage = Math.sqrt(
      tokenUsages.reduce((sum, tu) => sum + Math.pow(tu - avgTokenUsage, 2), 0) / tokenUsages.length
    );

    sessions.forEach((session: any) => {
      if (session.tokenUsage && Math.abs(session.tokenUsage - avgTokenUsage) > 2 * stdTokenUsage) {
        anomalies.push({
          timestamp: session.startTime,
          metric: 'Token使用量',
          value: session.tokenUsage,
          expected: Math.round(avgTokenUsage),
          severity: session.tokenUsage > avgTokenUsage + 3 * stdTokenUsage ? 'high' : 'medium',
