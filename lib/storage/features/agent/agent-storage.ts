...
import { AGENTS_KEY, SELECTED_AGENT_ID_KEY, LOCALLY_MODIFIED_AGENTS_KEY } from "../../shared/constants"
...
if (typeof window === 'undefined') {
  ...
  saveLocallyModifiedAgents = function(agentIds: string[]): void {
    try {
      localStorage.setItem(LOCALLY_MODIFIED_AGENTS_KEY, JSON.stringify(agentIds))
    } catch (error) {
  ...
  saveSelectedAgent = function(agentId: string): void {
    try {
      ...
      console.error("Failed to save selected agent ID to local storage:", error)
    }
  }
  ...
  loadSelectedAgentId = function(): string | null {
    try {
      ...
      console.error("Failed to load selected agent ID from local storage:", error)
      return null
    }
  }
}
