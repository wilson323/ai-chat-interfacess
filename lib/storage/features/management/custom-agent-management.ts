...
import { ErrorHandler, DatabaseError, FileSystemError, InternalError } from "@/lib/utils/error-handler"
import fs from 'fs'
...
export async function getCustomAgentStorageStats(): Promise<StorageStats> {
  try {
    ...
    for (const agent of customAgents) {
      totalSizeBytes += Buffer.byteLength(JSON.stringify(agent.toJSON()), 'utf8')
    }
    ...
    throw new InternalError('获取自研智能体存储统计失败', error as Error, { operation: 'getCustomAgentStorageStats' })
  }
}
...
export async function clearAllCustomAgentData(): Promise<boolean> {
  try {
    ...
    const standardError = ErrorHandler.handle(error, { operation: 'clearAllCustomAgentData' })
    throw new InternalError('清除自研智能体数据失败', error as Error, { operation: 'clearAllCustomAgentData' })
  }
}
...
export async function exportAllCustomAgentData(): Promise<CustomAgentData[]> {
  try {
    ...
    throw new InternalError('导出自研智能体数据失败', error as Error, { operation: 'exportAllCustomAgentData' })
  }
}
 ...
 * 导入自研智能体数据
 */
export async function importCustomAgentData(data: CustomAgentData[]): Promise<boolean> {
  try {
    ...
    throw new InternalError('导入自研智能体数据失败', error as Error, { operation: 'importCustomAgentData' })
  }
} 