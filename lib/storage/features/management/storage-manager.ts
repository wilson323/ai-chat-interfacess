import type { StorageMeta, StorageStats, StorageProvider, ChatSessionIndexItem } from "../../shared/types"
import {
  MESSAGES_PREFIX,
  CHAT_INDEX_KEY,
  STORAGE_META_KEY,
 ...
 * 检查存储是否接近限制
 */
export function isStorageNearLimit(provider: StorageProvider = defaultStorageProvider): boolean {
 ...
 * 紧急清理存储空间
 */
export function emergencyCleanup(provider: StorageProvider = defaultStorageProvider): void {
  try {
 ...
 * 清理旧聊天
 */
export function cleanupStorage(provider: StorageProvider = defaultStorageProvider): void {
  try {
    const meta = getStorageMeta(provider)
    const now = Date.now()
    ...
    const chatsByAge = [...meta.chatIds].sort(
      (a, b) => (meta.chatLastAccessed[a] || 0) - (meta.chatLastAccessed[b] || 0)
    )
    ...
    if (isStorageNearLimit(provider)) {
      ...
      const remainingChats = [...updatedMeta.chatIds].sort(
        (a, b) => (updatedMeta.chatLastAccessed[a] || 0) - (updatedMeta.chatLastAccessed[b] || 0)
      )
 ...
 * 获取存储统计信息
 */
export function getStorageStats(provider: StorageProvider = defaultStorageProvider): StorageStats {
  try {
    let totalSize = 0
    for (let i = 0; i < provider.length; i++) {
      const key = provider.key(i)
 ...
 * 清除所有聊天会话
 */
export function clearAllChatSessions(provider: StorageProvider = defaultStorageProvider): boolean {
  try {
    const allKeys: string[] = []
    for (let i = 0; i < provider.length; i++) {
      const key = provider.key(i)
 ...
 * 初始化存储系统
 */
export function initStorage(provider: StorageProvider = defaultStorageProvider): void {
  try {
    const meta = getStorageMeta(provider)
    const now = Date.now()
