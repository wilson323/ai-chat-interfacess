...
const ImageEditor = lazy(() => import('@/components/image-editor/image-editor-container'))

// 功能级别代码分割
const AgentManagement = lazy(() => import('@/components/admin/agent-management'))
...
export function useDebouncedState<T>(
  ...
  const setDebouncedState = useCallback((newState: T) => {
...
async function runPerformanceTest() {
      ...
      const observer = new PerformanceObserver((list) => {
        ...
        entries.forEach((entry) => {
          if (entry.entryType === 'navigation') {
            ...
            metrics.fcp = entry.domContentLoadedEventEnd - entry.navigationStart
          }
        })
  ...
  // 计算覆盖率
  const jsUsedBytes = jsCoverage.reduce((acc, entry) => acc + entry.text.length, 0)
  const jsTotalBytes = jsCoverage.reduce((acc, entry) => acc + entry.ranges.reduce((sum, range) => sum + range.end - range.start, 0), 0)
